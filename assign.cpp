...
assign::assign (string fileName, int nusr, int nmsg) {
...
void assign::init () {
   ...
   // read score
   this->readtext_usr_msg_score (nusr, nmsg);
...
// read data score & ????
void assign::readtext_usr_msg_score (int maxNusr, int maxNmsg) {
   ifstream inf;
   ...
   cout << " number of users: " << usrNum << " (max" << maxNusr << ")" << endl;
   cout << " number of messages: " << msgNum << " (max" << maxNmsg << ")" << endl;
   if (nusr > maxNusr || nmsg > maxNmsg) {
...
void assign::maxres(double *r1, double *r2, double *x) {
    ...
    *r2 = vector_amax(&x[nusr * nmsg], nmsg);
}
