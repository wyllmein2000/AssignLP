...
void assign::Constraint(double *x) {
    for (int i = 0; i < nusr * nmsg; i ++) {
	if (x[i] < 0) x[i] = 0.0;
    }
}
...
void assign::residual(double *y, double *x) {
    ...
    for (int iusr = 0; iusr < nusr; iusr ++) {
        ...
        for (int imsg = 0; imsg < nmsg; imsg ++) {
             int k = ks + imsg;
	     y[k] = score[k] - lambda * x[k] - gamma * (x_sum[imsg] - target[imsg]) - y_adj[k];
	}
...
double assign::misfit(double *x) {
    ...
    return y0 - 0.5* lambda * y1 - 0.5 * gamma * y2;
}
...
void assign::printResult(string outputFileName) {
   ...
   // if (fp) {
      ...
      for (int i = 0; i < nmsg; i ++) {
	  fp << setw(10) << i << setw(10) << a0[i] << " " << actual[i] << " " << actual_r[i] << " " << target[i] << endl;
      }
